...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-white border border-[#EAE7E1] rounded-xl px-3 py-2 shadow-sm">
      <div className="text-xs uppercase tracking-wider text-[#6B6A65]">
        {p.payload.label}
      </div>
      <div className="font-display font-semibold text-[#1C1C19]">
        {formatBRL(p.value)}
      </div>
    ...
    >
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        {data.map((d) => (
          <div
            key={d.type}
            className="rounded-xl bg-[#F9F8F6] border border-[#EAE7E1] px-3 py-2"
          >
            <div className="text-[10px] uppercase tracking-wider text-[#6B6A65]">
              {d.label}
            </div>
